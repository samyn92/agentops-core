...
func main() {
	...
	flag.BoolVar(&enableWebhooks, "enable-webhooks", false,
		"Enable admission webhooks for CRD validation. Requires cert-manager or manually provided TLS certificates.")
	...
	if enableWebhooks && len(webhookCertPath) > 0 {
	...
	mgrOptions := ctrl.Options{
		...
		LeaderElectionID:       "e58828d7.agentops.io",
	...
	if enableWebhooks {
		if err := (&agentsv1alpha1.Agent{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Agent")
			os.Exit(1)
		}
		if err := (&agentsv1alpha1.Channel{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Channel")
			os.Exit(1)
		}
		if err := (&agentsv1alpha1.Integration{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Integration")
			os.Exit(1)
		}
		if err := (&agentsv1alpha1.Provider{}).SetupWebhookWithManager(mgr); err != nil {
